...
- Document scan flow no longer depends on SDK-managed local document selection.
...
- **Breaking:** Removed `WDOVerificationState.documentsToScanSelect`.
- **Breaking:** Removed `WDOVerificationService.documentsSetSelectedTypes`.
- **Breaking:** Removed `WDOVerificationScanProcess` and `WDOScannedDocument`.
- **Breaking:** `WDOVerificationState.scanDocument` now carries `WDODocumentsStatus` instead of `WDOVerificationScanProcess`.
- Added public document scan models: `WDODocumentsStatus`, `WDODocument`, and `WDODocumentStatus`.
